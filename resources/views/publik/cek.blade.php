...
@section('konten')
    <section class="bg-navy-700 py-14 text-white">
        <div class="mx-auto max-w-3xl px-4">
            ...
            <p class="mt-2 text-white/80">Masukkan kode pendaftaran, nomor peserta, atau nomor HP yang didaftarkan.</p>
        </div>
    </section>
    ...
    <section class="mx-auto -mt-8 max-w-3xl px-4">
        <form method="POST" action="{{ route('cek.cari') }}" class="card">
            @csrf
            <label class="label" for="kata_kunci">Kode Pendaftaran / Nomor Peserta / No. HP</label>
                           ...
                           class="card flex flex-wrap items-center justify-between gap-3 transition hover:ring-2 hover:ring-langit-400">
                            <div>
                                ...
                                <p class="text-sm text-navy-900/60">
                                    {{ $peserta->kode_daftar }} &middot; {{ $peserta->no_bib }} &middot; {{ $peserta->label_paket }}
                                </p>
                            </div>
